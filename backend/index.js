...
if (process.env.NODE_ENV === 'production') {
    console.log("process.env.NODE_ENV work :", process.env.NODE_ENV);
    app.use(express.static(path.join(__dirname, '../frontend/build')));
    app.get('*', (req, res) => {
        res.sendFile(path.resolve(__dirname, '../frontend/build/index.html'));
    })
} else {
    console.log("process.env.NODE_ENV Not :", process.env.NODE_ENV);
}

mongoose.connect(process.env.DB_URL)
    .then(() => {
        console.log("Connected to MongoDB Successfully to:", process.env.DB_URL);
    })
    .catch((error) => {
